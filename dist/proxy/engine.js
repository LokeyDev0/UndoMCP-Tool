...
export class ProxyEngine {
    ...
    constructor(options) {
        ...
        if (this.dbManager) {
            ...
            this.undoController = new UndoController(this.dbManager, this.snapshotStore, this.schemaCache, this.inverseResolver, this.llmSolver);
        }
    }
                ...
                if (isFileModifying && filePath && this.snapshotStore && actionId) {
                    try {
                        ...
                        if (fs.existsSync(absolutePath)) {
                            ...
                            const preSnapshotId = this.snapshotStore.createSnapshot(actionId, absolutePath, content, 'pre');
     ...
     */
    async handleUndoAction(args) {
        const actionIds = args.action_ids || [];
        ...
        for (const actionId of actionIds) {
            ...
            try {
                this.dbManager.updateActionState(actionId, 'undone', new Date().toISOString());
                finalResults.push({
                    actionId,
                    success: true,
