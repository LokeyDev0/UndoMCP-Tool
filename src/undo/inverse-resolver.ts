 ...
 */
const ID_FIELD_PATTERNS: RegExp[] = [
  /^id$/i,
  /^uid$/i,
  /^_id$/i,
  /^\$id$/i,
...
export class InverseResolver {
  ...
  resolve(action: Action): InverseResolution | null {
    ...
    if (fileShadow) return fileShadow;
  ...
  private resolveFileShadow(action: Action): InverseResolution | null {
    if (action.actionType !== 'file_change') return null;
    ...
    if (!action.preSnapshotId) return null;
  ...
  private resolveVerbPair(action: Action): InverseResolution | null {
   ...
   */
  private mapParams(action: Action, inverseToolName: string): Record<string, any> | null {
    ...
    for (const param of allInverseParams) {
      if (result[param] !== undefined) continue; // already filled
      const value = this.findValueForParam(param, idFields, resultData, originalParams);
      if (value !== undefined) {
        result[param] = value;
      }
   ...
   */
  private extractIdFields(data: Record<string, any>): Map<string, any> {
  ...
  private findValueForParam(
    ...
    if (ID_FIELD_PATTERNS.some(p => p.test(targetParam))) {
      if ('id' in resultData) return resultData.id;
      if ('_id' in resultData) return resultData._id;
      if (idFields.size === 1) {
   ...
   */
  private resolvePatchArchive(action: Action): InverseResolution | null {
    if (!action.toolName) return null;
    const toolName = action.toolName;
    ...
    for (const schema of allSchemas) {
      ...
      let deleteParam: string | null = null;
      for (const propName of Object.keys(properties)) {
      ...
      if (!deleteParam) continue;

      const mappedIdValue = this.findValueForParam(
        idParam,
        this.extractIdFields(action.resultData || {}),
        action.resultData || {},
        action.parameters || {}
      );
      if (mappedIdValue === undefined) continue;

      const inverseParams: Record<string, any> = {
      ...
      for (const req of required) {
        if (req === idParam || req === deleteParam) continue;
      ...
      if (!satisfiedAllRequired) continue;
