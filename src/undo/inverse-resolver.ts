...
/**
 * Each verb-pair entry maps a forward verb prefix regex to an inverse verb prefix.
 * For example, `create_*` → `delete_*`.
 */
interface VerbPair {
...
export class InverseResolver {
   ...
   */
  public resolve(action: Action): InverseResolution | null {
   ...
   */
  public resolveForTurn(actions: Action[]): (InverseResolution | null)[] {
  ...
  private resolveVerbPair(action: Action): InverseResolution | null {
    ...
    for (const pair of VERB_PAIRS) {
      if (!pair.forwardPattern.test(toolName)) continue;

      // Build the candidate inverse tool name
      const inverseName = toolName.replace(
        new RegExp(`^${pair.forwardPrefix}`),
        pair.inversePrefix
      );
