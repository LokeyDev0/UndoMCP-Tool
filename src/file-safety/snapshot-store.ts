...
export class SnapshotStore {
  ...
  createSnapshot(
    ...
    filePath: string,
    content: Buffer,
    role: 'pre' | 'post' | 'baseline'
  ): string {
    if (content.length > MAX_SNAPSHOT_SIZE) {
      console.error(
        `[undomcp] Skipping snapshot for ${filePath}: file size ${content.length} exceeds ${MAX_SNAPSHOT_SIZE} byte limit.`
      );
  ...
  getSnapshotContent(snapshotId: string): Buffer | null {
    ...
    try {
      ...
      console.error(
        `[undomcp] Failed to decompress snapshot ${snapshotId}: ${err.message}`
      );
      return null;
    }
  }
