...
export async function resolveConflictPrompt(
  ...
  return new Promise((resolve) => {
    ...
    const isInteractive = usingDevice || (process.stdin as any).isTTY;
    if (!isInteractive) {
      console.warn(
        `[undomcp] Conflict in "${fileName}" but running in non-interactive environment. Defaulting to Exit.`
      );
