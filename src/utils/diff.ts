...
export function generateUnifiedDiff(
  ...
  const linesWithIndex: IndexedDiffLine[] = diffs.map((d) => {
    const item: IndexedDiffLine = {
      type: d.type,
      content: d.content,
      oldLineNum: d.type !== 'added' ? oldLineNum : null,
      newLineNum: d.type !== 'removed' ? newLineNum : null,
    };
    if (d.type !== 'added') oldLineNum++;
    if (d.type !== 'removed') newLineNum++;
    return item;
  });
  ...
  for (const hunk of hunks) {
    ...
    for (const hl of hunkLines) {
      if (hl.oldLineNum !== null) {
        if (oldStart === 0) oldStart = hl.oldLineNum;
        oldLength++;
      }
    }
    ...
    for (const hl of hunkLines) {
      if (hl.newLineNum !== null) {
        if (newStart === 0) newStart = hl.newLineNum;
        newLength++;
      }
    }
