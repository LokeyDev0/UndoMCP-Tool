...
export function runTui(
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    let cursorIndex = 0;
    let previewItem: TuiItem | null = null;
    ...
    function getPreviewText(item: TuiItem): string {
      if (item.type === 'turn') {
    ...
    const onKeypress = async (str: string, key: any) => {
      ...
      if (key.name === 'up' || key.name === 'k') {
        ...
        if (!item.alreadyUndone) {
          ...
          if (item.type === 'turn') {
            const turnActions = items.filter(
              (i) => i.type === 'action' && i.action?.turnId === item.id
            );
            turnActions.forEach((act) => {
            ...
            if (turnItem) {
              const siblingActions = items.filter(
                (i) => i.type === 'action' && i.action?.turnId === turnItem.id
              );
        ...
        const selectedActions = items.filter(
          (i) => i.type === 'action' && i.selected && !i.alreadyUndone
        );
        ...
        const conflictResolver = async (filePath: string): Promise<'exit' | 'overwrite'> => {
          ...
          return new Promise<'exit' | 'overwrite'>((resolvePrompt) => {
            rl.question(
              `[Conflict] File "${filePath}" has been modified externally. Overwrite? (y/N): `,
              (answer) => {
                rl.close();
